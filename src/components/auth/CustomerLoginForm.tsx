...
export function CustomerLoginForm() {
  ...
  const onSubmit = async (data: CustomerLoginInput) => {
    try {
      ...
      if (!res.ok) {
        toast.error(getErrorMessage(json.error, "Đăng nhập thất bại"));
        return;
      }
  ...
  return (
    <div className="bg-[#141824] rounded-3xl border border-white/15 p-7 sm:p-9 w-full max-w-md shadow-[0_0_50px_rgba(0,0,0,0.8)] backdrop-blur-xl relative overflow-hidden text-white">
      ...
      <div className="flex flex-col items-center mb-6 relative z-10">
        ...
        <p className="text-gray-400 text-xs mt-1 font-medium">Đăng nhập tài khoản Collector</p>
      </div>
      ...
      <div className="mt-6 pt-4 border-t border-white/10 text-center text-xs text-gray-400 relative z-10">
        Chưa có tài khoản?{" "}
        <Link href="/auth/register" className="text-[#E05638] hover:text-red-400 hover:underline font-extrabold transition-colors">
          Đăng ký ngay
        </Link>
      </div>
    </div>
